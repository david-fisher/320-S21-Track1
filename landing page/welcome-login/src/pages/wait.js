...
export default function Wait() {
  ...
  const getData = () => {
    ...
    function onSuccess(response) {
      const finishedScenarios = response.data.filter(
        (data) => data.IS_FINISHED,
      );
      const unfinishedScenarios = response.data.filter(
        (data) => !data.IS_FINISHED,
      );
  ...
  test().then((receivedData) => {
    console.log('abc ');
    console.log(receivedData);
    window.location.href = receivedData.affliation === 'Student'
      ? '/simulator'
      : receivedData.affliation === 'Employee'
        ? '/editor'
        : null;
  });
