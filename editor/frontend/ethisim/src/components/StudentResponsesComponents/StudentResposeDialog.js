...
import {
  Drawer, Box, Button, Typography, Dialog,
...
export default function Data({ open, selectedResponseData, handleClose }) {
  ...
  const fetchedStudentResponses = mockFinishedScenarioStudentResponse;
  const reflectionResponses = fetchedStudentResponses.reflections;
  let actionResponses = fetchedStudentResponses.actions;
  actionResponses = actionResponses.reduce(
    (result, currentValue) => addPage(
      result,
      currentValue.id,
      currentValue.title,
      'Action',
  ...
  const studentResponsePages = reflectionResponses.reduce(
    (result, currentValue) => addPage(
      result,
      currentValue.id,
      currentValue.title,
