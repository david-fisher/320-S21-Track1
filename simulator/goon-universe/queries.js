...
function getScenarios(studentID, callback) {
  const thisQuery = 'select scenario.id, scenario.name, scenario.description, scenario.due_date from scenario, partof, enrolled where enrolled.student_id = $1 and enrolled.course_id = partof.course_id and partof.scenario_id = scenario.id ';
...
function getPlainPage(scenarioID, order, callback) {
  const thisQuery = `select pages.body_text from pages where pages.order = ${order}and scenario_id = $1`;
...
function getConversationTaskPage(scenarioID, callback) {
  const thisQuery = 'select pages.body_text from conversation_task, pages where conversation_task.page_id = pages.id and pages.scenario_id = $1';
  pool.query(thisQuery, [scenarioID], (error, results) => {
    if (error) {
      throw error;
    }
...
function getAuthenticatedInstructorDashboardSummary(instructorID, callback) {
  const thisQuery = 'select scenario.id, scenario.name, scenario.description, scenario.due_date from scenario, partof, instructs where instructs.instructor_id = $1 and instructs.course_id = partof.course_id and partof.scenario_id = scenario.id ';
...
function getStudentsSummary(scenarioID, callback) {
  const thisQuery = 'select enrolled.student_id, submission.id from scenario, partof, enrolled, submissions where scenario.id = $1 and enrolled.course_id = partof.course_id and partof.scenario_id = scenario.id and submissions.scenario_id = scenario.id and submissions.user_id = enrolled.student_id';
...
async function getReflectResponse(studentID, scenarioID, pageOrder) {
  ...
  const testIDExistenceQuery = 'SELECT users.id, scenario.id FROM users, scenario WHERE users.id=$1 AND scenario.id=$2';
  const getReflectQuery = 'SELECT prompt_response.response, prompt_response.prompt_num FROM prompt_response, response, submissions, pages WHERE pages.order=$3 AND response.page_id=pages.id AND response.id=prompt_response.id AND response.submission_id=submissions.id AND submissions.user_id=$1 AND submissions.scenario_id=$2 AND pages.scenario_id=$2';
  try {
    await client.query('BEGIN');
    const existenceResult = await client.query(testIDExistenceQuery, [studentID, scenarioID]);
    if (existenceResult.rows.length !== 1) {
      throw new RangeError('IDs invalid');
    }
    const reflectResult = await client.query(getReflectQuery, [studentID, scenarioID, pageOrder]);
...
async function getStakeholdersHelper(scenarioID) {
  ...
  const stakeholderLimitResult = pool.query(stakeholderLimitQuery, [scenarioID]);
  const stakeholderListResult = pool.query(stakeholderListQuery, [scenarioID]);
  const stakeholderList = (await stakeholderListResult).rows;
  if (stakeholderList.length === 0) {
    return {};
  }
  const stakeholderLimit = await stakeholderLimitResult;
  const resultObject = {
...
function getName(userID, callback) {
  const thisQuery = 'select users.full_name from users where id =$1';
...
function getCourseInfo(courseID, callback) {
  const thisQuery = 'select webpage, name, semester from courses where id =$1';
...
function getInstructorInfo(instructorID, callback) {
  const thisQuery = 'select full_name, email, webpage, course_id from instructs, users where instructs.instructor_id =$1 and users.id=$1';
  pool.query(thisQuery, [instructorID], (error, results) => {
    if (error) {
      throw error;
    }
...
function addUser(fullName, email, callback) {
  const thisQuery = 'insert into users(full_name, email) values ($1 , $2)';
...
function addCourse(coursePage, courseName, semester, callback) {
  const thisQuery = 'insert into courses(webpage, name, semester) values ($1, $2, $3)';
...
async function addReflectionResponse(studentID, input, promptNum, scenarioID, timestamp, page_order) {
  ...
  try {
    ...
    const pageID = pageSelection.rows[0].id;
    ...
    const submissionID = submissionSelection.rows[0].id;
    ...
    const responseID = responseCreation.rows[0].id;
...
function scenarioExists(scenarioID) {
  ...
  const thisQuery = 'select scenario.id from scenario where scenario.id = $1';
...
async function createScenario(courseID, name, due_date, description, additional_data, callback) {
  ...
  try {
    ...
    const scenarioID = scenarioInsert.rows[0].id;
...
function setScenarioStatus(scenarioID, scenarioStatus) {
  ...
  const thisQuery = 'UPDATE scenario SET scenario.status = $2 WHERE scenario.id = $1';
...
async function scenarioPageExists(order, type, scenarioID) {
  // returns pageID
  const thisQuery = 'select pages.id from pages, scenario where pages.scenario_id = $1 and pages.order = $2 and pages.type = $3';
  let pageID = -1;
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const pageResult = await client.query(thisQuery, [scenarioID, order, type]);
...
async function createPage(order, type, body_text, scenarioID) {
  ...
  const thisQuery = 'insert into pages values(DEFAULT, $1, $2, $3, $4)';
  const client = await pool.connect();
  let pageID = -1;
  try {
    await client.query('BEGIN');
...
async function getPageID(scenarioID, order) {
  ...
  const pageIDQuery = 'select * from pages where pages.scenario_id = $1 and pages.order = $2';
  const client = await pool.connect();
  let pageID = -1;
  try {
    const pageIDresult = client.query(pageIDQuery, [scenarioID, order]);
...
async function addIntroPage(scenarioID, body_text, callback) {
  const pageID = await addPlainPage(scenarioID, body_text, INTROPAGE);
  callback(SUCCESS);
  return pageID;
}

async function addTaskPage(scenarioID, body_text, callback) {
  const pageID = await addPlainPage(scenarioID, body_text, TASKPAGE);
  callback(SUCCESS);
  return pageID;
}

async function addSummaryPage(scenarioID, body_text, callback) {
  const pageID = await addPlainPage(scenarioID, body_text, SUMMARY_PAGE);
  callback(SUCCESS);
  return pageID;
}
async function addConclusionPage(scenarioID, body_text, callback) {
  const pageID = await addPlainPage(scenarioID, body_text, CONCLUSIONPAGE);
  callback(SUCCESS);
  return pageID;
}
...
async function addReflectPage(scenarioID, body_text, prompts, ORDER) {
  const thisQuery = 'insert into prompt values($1, $2, DEFAULT)';
  let pageID = -1;
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
...
async function addConvTaskPage(scenarioID, body_text, convLimit, callback) {
  ...
  const thisQuery = 'insert into conversation_task values($1, $2)';
  let pageID = -1;
  const client = await pool.connect();
  try {
    client.query('BEGIN');
...
async function addStakeholder(scenarioID, name, designation, description, callback) {
  ...
  // insert stakeholder
  const thisQuery = 'insert into stakeholders values(DEFAULT, $1, $2, $3, "", $4, $5) returning id;';
  const client = await pool.connect();
  try {
    client.query('BEGIN');
    if (scenarioExists(scenarioID)) {
      // create page object (checks for c)
      const conv_task_pageID = getPageID(scenarioID, CONVERSATION);
...
async function addStakeholderConversations(stakeholderID, conv_ques_text_array) {
  ...
  const thisQuery = 'insert into conversation values(DEFAULT, $1, $2, $3)';
  const client = await pool.connect();
  try {
    client.query('BEGIN');
    // insert conversations from array
    for (const conv of conv_ques_text_array) {
...
function addActionPage(scenarioID, order, body_text, QA_array) {
  try {
    if (scenarioExists(scenarioID)) {
      const pageID = createPage(order, TYPE_MCQ, body_text, scenarioID);
      addMCQ(scenarioID, FINAL_ACTION, QA_array);
    } else {
      throw RangeError(`ScenarioID ${scenarioID} does not exist`);
    }
...
async function addMCQ(scenarioID, order, QA_array) {
  ...
  const pageID = getPageID(scenarioID, order);
  try {
    for (const QA of QA_array) {
      const Q_ID = addMCQQuestion(QA[0], pageID);
...
async function addMCQQuestion(question, mcq_id) {
  ...
  const thisQuery = 'insert into question values(DEFAULT, $1, $2) RETURNING id';
  let questionID = -1;
  const client = pool.connect();
  try {
    await client.query('BEGIN');
    const query_result = pool.query(thisQuery, [question, mcq_id]);
...
async function addMCQOptions(option_array, question_id) {
  // TODO check for invalid parameters
  const thisQuery = 'insert into mcq_option values(DEFAULT, $1, $2)';
  const client = pool.connect();
  try {
    (await client).query('BEGIN');
...
function getStakeholderDescriptions(scenarioID) {
  // TODO check for invalid parameters
  if (scenarioExists()) {
    const thisQuery = 'select stakeholders.id, stakeholders.description from stakeholders where stakeholders.scenario_id=$1';
    pool.query(thisQuery, [scenarioID], (error, results) => {
      if (error) {
        throw error;
      }
...
function getInitReflectPage(scenarioID, callback) {
  const thisQuery = `select pages.body_text, prompt.prompt, prompt.prompt_num from pages, prompt where pages.id = prompt.page_id and pages.order = ${INITIAL_REFLECTION} and scenario_id = $1`;
  pool.query(thisQuery, [scenarioID], (error, results) => {
    if (error) {
      throw error;
    }
...
function getMidReflectPage(scenarioID, callback) {
  const thisQuery = `select pages.body_text, prompt.prompt, prompt.prompt_num from pages, prompt where pages.id = prompt.page_id and pages.order = ${MIDDLE_REFLECTION} and scenario_id = $1`;
  pool.query(thisQuery, [scenarioID], (error, results) => {
    if (error) {
      throw error;
    }
...
function getFinalReflectPage(scenarioID, callback) {
  const thisQuery = `select pages.body_text, prompt.prompt, prompt.prompt_num from pages, prompt where pages.id = prompt.page_id and pages.order =${FINAL_REFLECTION}and scenario_id = $1`;
  pool.query(thisQuery, [scenarioID], (error, results) => {
    if (error) {
      throw error;
    }
...
function getActionPageQuestionsAndChoices(scenarioID, pageOrder, callback) {
  const thisQuery = 'SELECT question.id AS question_id, question.question, mcq_option.id AS option_id, mcq_option.option FROM pages, mcq, question, mcq_option WHERE pages.scenario_id = $1 AND pages.order = $2 AND mcq.page_id = pages.id AND question.mcq_id = mcq.page_id AND mcq_option.question_id = question.id ORDER BY question_id';
  pool.query(thisQuery, [scenarioID, pageOrder], (error, results) => {
    ...
    if (results.rows.length !== 0) {
      ...
      for (i = 1; i < results.rows.length; i++) {
        const question_id_current = questionObject.question_id;
        const question_id_new = results.rows[i].question_id;
        /*
...
async function addMCQResponse(studentID, questionID, choiceID, scenarioID, timestamp, page_order) {
  ...
  try {
    ...
    const responseCreation = await client.query(insertResponseQuery, [submissionID, pageID, timestamp]);
    const responseID = responseCreation.rows[0].id;
    const existenceQueryResult = await client.query(IDExistenceQuery, [responseID, questionID, choiceID]);
...
async function getStakeholderHistoryHelper(studentID, scenarioID) {
  ...
  try {
    ...
    const responseID = responseSelection.rows[0].id;
    const convIdSelection = await client.query(getConvsFromResponse, [responseID]);
    const convIDs = convIdSelection.rows;
    // RETURNING clause returns ID at the same time
...
async function convIdsToStakeholderIds(convIds) {
  ...
  try {
    for (let i = 0; i < convIds.length; i++) {
      const convId = convIds[i].conversation_id;
      const thisStakeholderID = await client.query(getStakeholderID, [convId]);
      const stakeholderID = thisStakeholderID.rows[0].stakeholder_id;
...
async function getScenarioIssueCoverageMatrix(scenarioID, callback) {
  ...
  const client = await pool.connect();
  try {
    const matrix = await client.query(matrixquery, [scenarioID]);
...
function getStakeholderConversation(stakeholderID, callback) {
  const thisQuery = 'select conversation.question, conversation.conversation_text from conversation where conversation.stakeholder_id=$1';
