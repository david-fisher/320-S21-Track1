...
router.route('/scenarios')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(studentID)) {
      ...
      db.getScenarios(studentID, (result) => {
        if (result.length == 0) {
...
router.route('/scenarios/intro')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getIntroPage(scenarioID, (result) => {
        if (result.length == 0) {
...
router.route('/scenarios/task')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getTaskPage(scenarioID, (result) => {
        if (result.length == 0) {
...
router.route('/scenarios/starttogatherinfo')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getInitActionSubsequentPage(scenarioID, (result) => {
        if (result.length == 0) {
...
router.route('/scenarios/initialReflection')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getInitReflectPage(scenarioID, (result) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      for (prompt_num in data) {
        if (!isnumber(prompt_num)) {
          ...
          db.addInitReflectResponse(studentID, input, prompt_num, scenarioID, timestamp, (result) => {
            if (result.length === 0) {
              no_error = false;
            }
            // else{
...
router.route('/scenarios/initialReflection/response')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(studentID)) {
      ...
      db.getInitReflectResponse(studentID, scenarioID, (result) => {
        if (result == null) {
...
router.route('/scenarios/initialAction')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getInitActionPageQuestionsAndChoices(scenarioID, (result) => {
        if (result.length == 0) {
      ...
      for (questionID in data) {
        if (!isnumber(questionID)) {
          ...
          if (!isnumber(choiceID)) {
            ...
            db.addInitActionResponse(studentID, questionID, choiceID, scenarioID, timestamp, (result) => {
...
router.route('/scenarios/initialAction/response')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(studentID)) {
      ...
      db.getInitActionResponse(studentID, scenarioID, (result) => {
        if (result.length == 0) {
...
router.route('/scenarios/finalAction')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getFinalActionPageQuestionsAndChoices(scenarioID, (result) => {
        if (result.length == 0) {
      ...
      for (questionID in data) {
        if (!isnumber(questionID)) {
          ...
          if (!isnumber(choiceID)) {
            ...
            db.addFinalActionResponse(studentID, questionID, choiceID, scenarioID, timestamp, (result) => {
              if (result === 'scenario/status ID error') {
                no_error = false;
                // res.status(404).json({error: `student ID or scenario ID does not exist in database`})
              } else if (result === 'response/question/choice ID error') {
...
router.route('/scenarios/finalAction/response')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(studentID)) {
      ...
      db.getFinalActionResponse(studentID, scenarioID, (result) => {
        if (result.length == 0) {
...
router.route('/scenarios/consequences')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getConsequencesPage(scenarioID, (result) => {
        if (result.length == 0) {
...
router.route('/scenarios/stakeholders/history')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getStakeholderHistory(studentID, scenarioID, (result) => {
        /* if(result.length == 0){
...
router.route('/scenarios/stakeholders')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getStakeholders(scenarioID, (result) => {
        if (Object.entries(result).length == 0) {
  ...
  .put((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      timestamp = new Date();
      db.addStakeholderChoice(studentID, scenarioID, stakeholderID, timestamp, (result) => {
        if (result.length === 0) {
...
router.route('/scenarios/stakeholders/conversation')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(stakeholderID)) {
      ...
      db.getStakeholderConversation(stakeholderID, (result) => {
        if (Object.entries(result).length == 0) {
...
router.route('/scenarios/middleReflection')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getMidReflectPage(scenarioID, (result) => {
        if (Object.entries(result).length == 0) {
    ...
    if (!isnumber(scenarioID)) {
      ...
      for (prompt_num in data) {
        if (!isnumber(prompt_num)) {
          ...
          input = data[prompt_num];
          db.addMidReflectResponse(studentID, input, prompt_num, scenarioID, timestamp, (result) => {
            if (result.length === 0) {
              no_error = false;
            }
            // else{
...
router.route('/scenarios/middleReflection/response')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(studentID)) {
      ...
      db.getMidReflectResponse(studentID, scenarioID, (result) => {
        if (result == null) {
...
router.route('/scenarios/finalReflection')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getFinalReflectPage(scenarioID, (result) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      for (prompt_num in data) {
        if (!isnumber(prompt_num)) {
          ...
          input = data[prompt_num];
          db.addFinalReflectResponse(studentID, input, prompt_num, scenarioID, timestamp, (result) => {
            if (result.length === 0) {
              no_error = false;
            }
            // else{
...
router.route('/scenarios/finalReflection/response')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(studentID)) {
      ...
      db.getFinalReflectResponse(studentID, scenarioID, (result) => {
        if (result == null) {
          res.status(404).json({ error: 'No middle reflection response found with one or both of the ID\'s' });
        } else {
          res.status(200).json(result);
...
router.route('/scenarios/conclusion')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getConclusionPage(scenarioID, (result) => {
        if (result.length == 0) {
          res.status(404).json({ error: `No scenario found with scenarioid: ${scenarioID}` });
        } else {
          res.status(200).json(result);
  ...
  .put((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.addConclusionResponse(studentID, scenarioID, data, (result) => {
        if (result.length === 0) {
          res.status(404).json({ error: 'student ID or scenario ID does not exist in database' });
        } else {
          res.status(200).send(result);
...
router.route('/scenarios/feedback')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getFeedback(scenarioID, studentID, (result) => {
        if (result.length == 0) {
...
router.route('/scenarios/summary')
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getSummary(scenarioID, studentID, (result) => {
        if (result.length == 0) {
...
router.route('/scenarios/lastPage')
  ...
  .get((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.getLastPage(scenarioID, studentID, (result) => {
        if (result.length == 0) {
  ...
  .put((req, res) => {
    ...
    if (!isnumber(scenarioID)) {
      ...
      db.addLastPage(studentID, scenarioID, data, (result) => {
        if (result.length === 0) {
          res.status(404).json({ error: 'student ID or scenario ID does not exist in database' });
        } else {
          res.status(200).send(result);
